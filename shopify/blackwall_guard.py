"""
Black_Wall guard for Shopify agent actions (Shopify agent tooling / Admin API wrapper).

Commerce agents touch real money and a live storefront — refunds, order
cancellations, price changes, publishing. Catch the wrong one BEFORE it hits the store:
...
def guard(action, context=None, on_confirm=None, fail_open=False):
    """Decorate a Shopify action (refund, cancel, price change, publish) so Black_Wall
    vets it before it touches the store.

        @guard(action="refund_order", context={"agent_role": "support agent"})
        def refund_order(order_id, amount_usd, reason=""): ...

    Default is fail-closed. Works on sync and async functions.
    """
    def deco(fn):
...
# ── Example (runs without Shopify tooling installed) ──────────────────────────
if __name__ == "__main__":
    ...
    def refund_order(order_id: str, amount_usd: float, reason: str = "") -> str:
        # In real use this calls the Shopify Admin API / agent tooling. Stubbed here.
        return f"refunded ${amount_usd} on order {order_id}"

    try:
        # A $4,800 refund on what should be a small order — 10x error. Black_Wall should hold.
        print(refund_order(order_id="1234", amount_usd=4800, reason="customer request"))
