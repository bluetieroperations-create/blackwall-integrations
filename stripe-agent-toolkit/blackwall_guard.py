"""
Black_Wall guard for money-moving agent actions (Stripe Agent Toolkit / any Stripe wrapper).

Put a pre-action risk check in front of the actions that move money — payments,
payouts, refunds, invoices. The agent's proposed action is risk-checked BEFORE it
hits Stripe:
...
def guard(action, context=None, on_confirm=None, fail_open=False):
    """Decorate a money-moving function so Black_Wall vets it before it calls Stripe.

        @guard(action="make_payment", context={"agent_role": "AP bot"})
        def pay_vendor(amount_usd: float, to: str) -> str:
            # ... your Stripe call (PaymentIntent, Payout, etc.) ...

    For money actions, fail-closed (the default) is the safe posture. Works on sync
    and async functions. Pass fail_open=True only if you truly want the payment to
    proceed when the check is unreachable (not recommended for money).
    """
    def deco(fn):
...
# ── Example (runs without Stripe installed) ───────────────────────────────────
if __name__ == "__main__":
    ...
    def pay_vendor(amount_usd: float, to: str, memo: str = "") -> str:
        # In real use this calls Stripe (PaymentIntent / Payout). Here it's stubbed.
        return f"paid ${amount_usd} to {to}"

    try:
        # $48k to an unverified recipient — Black_Wall should hold/stop before Stripe is touched.
        print(pay_vendor(amount_usd=48000, to="new-vendor@unknown.com", memo="invoice"))
