"""
Black_Wall guard for money-moving agent actions (PayPal Agent Toolkit / any PayPal wrapper).

Put a pre-action risk check in front of the actions that move money — payments,
payouts, refunds, invoices — so a wrong amount or unverified recipient is caught
BEFORE it hits PayPal:
...
def guard(action, context=None, on_confirm=None, fail_open=False):
    """Decorate a money-moving function so Black_Wall vets it before it calls PayPal.

        @guard(action="make_payment", context={"agent_role": "AP bot"})
        def send_payout(amount_usd: float, to: str): ...
...
# ── Example (runs without PayPal installed) ───────────────────────────────────
if __name__ == "__main__":
    ...
    def send_payout(amount_usd: float, to: str, note: str = "") -> str:
        # In real use this calls the PayPal Agent Toolkit (Payouts / Orders). Stubbed here.
        return f"paid ${amount_usd} to {to}"

    try:
        # $48k payout to an unverified recipient — Black_Wall should hold/stop before PayPal is touched.
        print(send_payout(amount_usd=48000, to="new-payee@unknown.com", note="invoice"))
