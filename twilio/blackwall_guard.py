"""
Black_Wall guard for Twilio agent actions (SMS / voice / WhatsApp).

Comms agents can fire a message to the wrong number, leak PII, or blast a whole list
by mistake. Catch it BEFORE Twilio sends:
    AUTO            -> the message sends
...
def guard(action, context=None, on_confirm=None, fail_open=False):
    """Decorate a Twilio send (SMS/voice/WhatsApp) so Black_Wall vets it before it sends.

        @guard(action="send_sms", context={"agent_role": "notifications agent"})
        def send_sms(to, body, recipient_count=1): ...

    Pass a recipient_count (or the recipient list) so Black_Wall can catch mass-sends.
...
# ── Example (runs without twilio installed) ───────────────────────────────────
if __name__ == "__main__":
    ...
    def send_sms(to: str, body: str, recipient_count: int = 1) -> str:
        # In real use this calls the Twilio API. Stubbed here.
        return f"sent SMS to {recipient_count} recipient(s)"

    try:
        # A "one reminder" intent that fans out to 8,400 numbers — mass-send. Black_Wall should hold.
        print(send_sms(to="messaging-service", body="Reminder: ...", recipient_count=8400))
